...
async function extractData() {
      ...
      "requestItems": [
        {
          ...
          "files": [
            {
              "name": `${file.name}`,
              "value": `${ocrResponse.resultItems[0].files[0].value}`,
              "contentType": `${ocrResponse.resultItems[0].files[0].contentType}`,
              "fileType": 'pdf'
            }
          ]
        }
  ...
  //   return
  // }
// console.log((JSON.stringify(UimData.value.nodeList, "", 2)));
...
async function getFiles(){
  ...
  const url = `${properties.base_url}/cms/instances/folder/cms_folder/${folderId}/items`
  ...
  console.log((JSON.stringify(data, "", 2)));
    ...
    let collection = data._embedded.collection;
    let tempFiles = [];

    // Loop through each child in the collection
    for (let i = 0; i < collection.length; i++) {
      ...
      // Check if the child has renditions and _links with urn:eim:linkrel:download-media
      if (child && child._links && child._links['urn:eim:linkrel:download-media']) {
        // Access the urn:eim:linkrel:download-media value
        let downloadMedia = child._links['urn:eim:linkrel:download-media'].href;
        tempFiles.push({ name, downloadMedia });
      }
    }
  ...
  return (
    <main>
      <h1 className="ot2-sample-header">HealthGuard Demo</h1><br/>
      <div className="ot2-body">
      <div>
        <h3>Authentication API to obtain Access Token</h3>
      ...
      {savedFiles.map((savedFile, index) => (
        <div key={index}>
          <p>{savedFile.name}</p>
          <button onClick={() => downloadFile(savedFile.downloadMedia, savedFile.name)}>Download</button>
        </div>
      ))}
    </div>
      <hr align='left' />
      <div>
        <h3>Get Files</h3>
        <label htmlFor='folderName'>Provide a folder name: </label>&nbsp;
        <input type="text" name="folderName" size="35" onChange={getFolderName} /><br/><br/>
        <button onClick={createFolder}>Get Folder</button>&nbsp;&nbsp;
        <button onClick={getFiles}>Get Files</button><br/><br/>
      </div>
     
      <hr align='left' />
      <div>
        <h3>Upload File To Capture Service</h3>
      ...
      <hr align='left' />
      <div>
        <h3>Convert File into Text Searchable PDF</h3>
      ...
      <hr align='left' />
      <div>
        <h3>Uploaded PDF to Content Storage</h3>
        <label><b>Step 1: </b></label>&nbsp;
      ...
      <hr align='left' />
      <div>
        <h3>Organize File in the folder</h3>
      ...
      <hr align='left' />
      <div>
        <h3>Create and upload metadata for the PDF file</h3>
        <button onClick={createMetadataForFile}>Upload Metadata</button>
      ...
      <hr align='left' />
      <div>
        <h3>Extract Data from PDF File</h3>
        <button onClick={extractData}>Extract the PDF File</button><br/><br/>
